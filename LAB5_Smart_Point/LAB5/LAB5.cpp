...
int main() {
    ...
    std::unique_ptr<Plane> p1 = std::make_unique<Plane>("Su-25", 1.5 );
    p1->print();
    std::cout << " " << std::endl;

    std::cout << "---Unique pointer move---" << std::endl;
    std::unique_ptr<Plane> p3 = std::make_unique<Plane>("F-16", 2.0 );
    std::unique_ptr<Plane> p2 = std::move(p3);
    p2->print();
    std::cout << " " << std::endl;

    std::cout << "---Shared pointer---" << std::endl;
    std::shared_ptr<Plane> s1 = std::make_shared<Plane>("Z-5", 1.8 );
    std::shared_ptr<Plane> s2 = s1;
    s1->print();
    s2->print();
